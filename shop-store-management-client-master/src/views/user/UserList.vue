<template>
  <div>
    ...
    <div class="option-button">
      ...
      <el-button @click.native="resetSearch" size="medium" type="danger"
        >重置
      </el-button>
      <el-button
      ...
      >
        <template v-for="item in tableItem">
          ...
          <el-table-column
            ...
            key="usertype"
            label="用户类型"
            v-if="item.name === 'usertype' && item.isShow"
            width="100"
          >
          ...
          >
            <template slot-scope="scope">
              ...
              <span class="status-success" v-if="scope.row.status === 0">{{
                statusMap[scope.row.status]
              }}</span>
              <span class="status-failed" v-else>{{
                statusMap[scope.row.status]
              }}</span>
            </template>
          </el-table-column>
          ...
          >
            <template slot-scope="scope">
              <el-button
                @click.native="showImgDialogFun(scope.row.avatarUrl)"
                size="mini"
                type="text"
        ...
        <el-table-column align="center" fixed="right" label="操作" width="400">
          <template slot-scope="scope">
            ...
            <el-button
              @click.native="showDeliveryAddress(scope.row)"
              size="mini"
              type="text"
              >查看收货地址
            </el-button>
    ...
    >
      ...
      >
        ...
        <el-table-column align="center" label="默认地址" width="70">
          <template slot-scope="scope">
    ...
    >
      <div class="reset-pwd">
        <span>新密码：</span> <span class="new-pwd">{{ newPwd }}</span>
        <el-button
          size="medium"
          type="text"
    ...
    <el-dialog :visible.sync="showImgDialog" top="50px" width="700px">
      <div class="dialog-img"><img :src="showImageUrl" alt="图片详情" /></div>
    </el-dialog>
    <el-dialog
    ...
    >
      <div class="edit-table-dialog">
        <div class="tips">
          ...
          <el-button @click="resetEditTable" size="mini" type="text">
            恢复默认
          </el-button>
        </div>
        <SlickList
      ...
      <span class="dialog-footer" slot="footer">
        <el-button @click="editTableDialog = false" size="small"
          >取 消</el-button
        >
        <el-button @click="submitEditTable" size="small" type="primary"
          >确 定</el-button
        >
      </span>
    </el-dialog>
    ...
    >
      ...
      >
        <el-form-item label="用户账号" prop="name">
          <el-input v-model="editUserForm.name" placeholder="请输入用户账号"></el-input>
        </el-form-item>
        <el-form-item label="用户名" prop="nickname">
          <el-input v-model="editUserForm.nickname" placeholder="请输入用户名"></el-input>
        </el-form-item>
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="submitEditUser" :loading="submitLoading">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async getUserList() {
      ...
      try {
        ...
        if (res.errorCode === 200) {
          // 合并管理员和普通用户数据
          let adminUsers = res.data.adminUserResult || [];
          let regularUsers = res.data.UserResult.rows || [];
          
          // 确保管理员用户数据包含必要字段
          adminUsers = adminUsers.map(admin => {
            return {
              ...admin,
              status: admin.status === null ? 0 : admin.status, // 如果状态为null，默认为正常
              usertype: 1 // 确保usertype为1
            };
          });
          
          // 确保普通用户数据包含usertype字段
          regularUsers = regularUsers.map(user => {
            return {
              ...user,
              usertype: 0 // 确保usertype为0
            };
          });
          
          // 合并数据
          this.tableData = [...adminUsers, ...regularUsers];
    ...
    resetPwd(row) {
      this.$confirm(
        `确定 重置用户 <span style="color:#409eff">${
          row.name
        }</span> 的密码吗？`,
        "提示",
        {
    ...
    editUser(row) {
      ...
      this.editUserForm.usertype = row.usertype;
    },
    // 关闭编辑用户弹窗
    closeEditUserDialog() {
    ...
    submitEditUser() {
      this.$refs.editUserForm.validate(async (valid) => {
        if (valid) {
          ...
          try {
            let res = await userApi.editUserInfo({
